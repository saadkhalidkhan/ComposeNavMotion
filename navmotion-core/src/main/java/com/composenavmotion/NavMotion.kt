...
object NavMotion {
    fun none(): NavMotionSpec = NavMotionSpec(
    ...
    fun custom(
        ...
        return NavMotionSpec(
            enterTransition = config.enter(),
            exitTransition = config.exit(),
            popEnterTransition = config.popEnter(),
            popExitTransition = config.popExit(),
            config = config,
        )
    }

    /**
     * Combines separate forward and backward [NavMotionSpec] instances for direction-aware navigation.
     *
